...
int glmFindOrAddTexture(GLMmodel* model, char* name,mycallback *call)
{
	...
	int procent = ((float)((float)model->numtextures*30/total_textures)/100);
	//*(call->end-call->start)+call->start; // Note: moved this into if, prevent crash on null call 
	if (call)
	{
		procent *= (call->end-call->start)+call->start;
